...
class CS2Analyzer:
    ...
    def load_data(self) -> None:
        ...
        try:
            self.df = pd.read_csv(
                self.file_path, skipinitialspace=True, engine="python"
            )
    ...
    def _calculate_metrics(
        self, series: Any, reverse: bool = False
    ) -> Dict[str, float]:
        import pandas as pd
        ...
        for p, base_label in CFG.PERCENTILE_MAP.items():
            label = (
                STR.LBL_MEDIAN
                if p == 0.5
                else f"{base_label} {STR.SUFFIX_LOW if reverse else STR.SUFFIX_HIGH}"
            )
    ...
    def display_report(self) -> None:
        ...
        for category, metrics in self.results.items():
            ...
            for label, val in metrics.items():
                color = Fore.WHITE
                if label in (STR.LBL_MEAN, STR.LBL_MEDIAN):
                    color = Fore.GREEN
                elif STR.SUFFIX_LOW in label or STR.SUFFIX_HIGH in label:
                    color = Fore.YELLOW
                elif label == STR.LBL_JITTER:
        ...
        print(
            f"  {Fore.WHITE}{STR.LBL_SERVER:20}: {self.server_diff:.2f} {STR.UNIT_MS}"
        )
